...
/**
 * @type { {hw: string, belacoder_path: string, srtla_path: string, bitrate_file: string, ips_file: string}}
 */
...
/** @type {{belacoder: string, srtla: string, belaUI: string, "BELABOX image": string?}} */
const revisions = {};
try
{
...
/**
 * @type {{password_hash: string, max_br: string, min_br: string, asrc: string, acodec: string, delay: string, pipeline: string, srt_latency: string, srt_streamid: string, srtla_addr: string, srtla_port: string, bitrate_overlay: string}}
 */
let config;
/** @type {string?} */
let sshPasswordHash;
...
function saveConfig ()
{
    ...
    const c = JSON.stringify(config, undefined, '\t');
    delete config.ssh_pass_hash;
...
function updateSensorsJetson ()
{
    ...
    try
    {
        const thermDevs = fs.readdirSync("/sys/class/thermal/");
        sensors['SoC temperatures'] = {};
        for (let i = 0; i < thermDevs.length; i++)
        {
            const sensor = thermDevs[i];

            if (sensor.startsWith("thermal_zone"))
            {
                let socTemp = fs.readFileSync(`/sys/class/thermal/${sensor}/temp`, 'utf8');
                let socType = fs.readFileSync(`/sys/class/thermal/${sensor}/type`, 'utf8');

                // PMIC doesnt give an actual reading
                // https://forums.developer.nvidia.com/t/pmic-temperature-in-tegrastats-output/75582/2
                if (socType.includes("PMIC")) {
                    continue;
                }else {
                    let socTempNum = parseInt(socTemp) / 1000;
                    // socTemp = `${socTemp.toFixed(1)} °C`;
                    sensors['SoC temperatures'][socType] = socTempNum;
                }
            }
        }
    } catch (err)
    {
        console.error("Error getting SoC temperatures:", err);
    };
...
function setBitrate (params)
{
    ...
    if (params.max_br < params.min_br || params.max_br > 12000) return null;

    config.max_br = params.max_br;
    config.min_br = Math.max(params.min_br, minBr);
    saveConfig();

    fs.writeFileSync(setup.bitrate_file, config.min_br * 1000 + "\n"
        + config.max_br * 1000 + "\n");

    spawnSync("killall", ['-HUP', "belacoder"], { detached: true });

    return { max_br: config.max_br, min_br: config.min_br };
}
...
async function updateConfig (conn, params, callback)
{
    ...
    if (latencyTmp != params.srt_latency || latencyTmp < 0 || latencyTmp > 10000)
...
function spawnStreamingLoop (command, args, cooldown = 100, errCallback)
{
    ...
    if (errCallback)
    {
        process.stderr.on('data', function (data)
        {
            ...
            if (!data.includes("gst_buffer_resize_range"))
            {
                console.log(data);
                errCallback(data);
            }
...
/*         spawnStreamingLoop(srtlaSendExec, [
        ...
        const belacoderArgs = [
            pipeline,
            srtlaAddr ?? '127.0.0.1',
            config.srtla_port ?? '9000',
            '-d', config.delay,
            '-b', setup.bitrate_file,
            '-l', config.srt_latency,
        ];
        console.log(belacoderExec, ...belacoderArgs);
        ...
        switch (type)
        {
            ...
            case 'bitrate':
                if (isStreaming)
                {
                    ...
                    if (br != null)
                    {
                        broadcastMsgExcept(conn, 'bitrate', bt);
                    }
                }
                break;
            case 'command':
